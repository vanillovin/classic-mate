...
function ClassicLikeButton({ classicId, className, serverLikeCount, isShowLikeCount = false }: Props) {
  ...
  return (
    ...
    >
      ...
      {isShowLikeCount && <span className='text-sm sm:text-base'>{likeCount}</span>}
    </button>
  );
}
